...
class App extends React.Component {
  ...
  deletedAllDone = async () => {
    try {
      const res = await axios.delete('http://localhost:1234/products/deleteAllDone')
      if (res.status === 200) {
        let undeleteList = this.state.dataList.filter(el => el.status === false);
        this.setState({ dataList: undeleteList });
      }
    }
    catch (err) {
    };
        ...
        <div className='row'>
          ...
          <div className='col'>
            <ItemList
              ...
              deletedAllDone={this.deletedAllDone}
